...
public:
    int minTimeToReach(vector<vector<int>>& moveTime) {
        ...
        pq.push(make_pair(0, make_pair(0, make_pair(0, 0))));
        vis[0][0] = 1;

        while(!pq.empty()){
            int time = pq.top().first;
            int stepTime = pq.top().second.first;
            ...
            for(int i=0;i<4;i++){
                ...
                if(isValid(newX, newY, vis)){
                    vis[newX][newY] = 1;
                    pq.push(make_pair(max(moveTime[newX][newY], time)+(stepTime%2+1), make_pair(stepTime+1, make_pair(newX, newY))));
                }
            }
